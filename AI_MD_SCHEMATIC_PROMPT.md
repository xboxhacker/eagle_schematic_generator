...
6. Nets and power rails
   ...
   - **CRITICAL — NEVER combine supply nets of different names**: Supply nets (VCC5, VCC12, VCC3V3, GND, VDD, VSS, etc.) are **distinct electrical nodes**. A single pin MUST appear on **exactly one** supply net. If J8.Pin1 is +5V, it goes on `Net VCC5` ONLY — never also on `Net VCC12` or any other supply. Listing the same pin in multiple supply nets (e.g., J8.Pin1 in both VCC5 and VCC12) will short 5V to 12V in the schematic and destroy the design. **Every connector pin, IC power pin, and passive tied to a rail must appear in exactly one supply net.**
