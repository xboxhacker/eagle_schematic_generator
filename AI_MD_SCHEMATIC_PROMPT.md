...
**Show your work**: Before the main schematic content, include a `## Datasheet Citations` section listing every datasheet you consulted, with manufacturer, part number, and document title or URL. Example: `- 74HC123: Nexperia 74HC123 datasheet (Dual retriggerable monostable multivibrator with reset), Rev. 11`. This proves you did not guess pin assignments.

**Strict datasheet pin usage**: For ALL ICs (U*) and non-passive components (connectors J*, MOSFETs Q*, etc.), use ONLY the pin numbers and pin names from the manufacturer datasheet. Never invent or approximate pin names. If the datasheet says Pin 13 is "2CLR" or "2RD", use that exact name. If it says "1~Q" (active-low), use that. Copy the pinout table or diagram from the datasheet verbatim into your pin listings.
...
1. Document outline
   ...
   - **Immediately after the summary**, add `## Datasheet Citations` listing every datasheet consulted (manufacturer, part number, document title). Example: `- 74HC123N: Nexperia 74HC123 datasheet, Rev. 11 — Pin 13 = 2CLR (clear input), Pin 4 = 1Q (output)`. This section proves pin assignments are datasheet-verified.
...
3. Component sections
   ...
   - **CRITICAL for ICs (U*)**: Use the **physical pin number** from the datasheet (1, 2, 3... 14, 16, etc.). The parenthetical name must be the **datasheet functional name** (e.g. 2CLR, 1Q, VCC, GND) and ideally match the Eagle library logical pin name (e.g. 1A, 1Y, 2~RD) so the generator can map correctly. Never invent names—copy them from the datasheet pinout table. Example: 74HC123 Pin 13 is "2CLR" or "2RD" in datasheets; Eagle may use `2~RD`. Use the datasheet name; the generator has aliases for common variants. For active-low pins, Eagle uses `~`; you may write without tilde—the generator matches both. **Cite the datasheet in your Datasheet Citations section for every IC.**
...
5. Pin-accuracy checklist (never invent pins)
   ...
   - **Connectors with power/common pins**: Many connectors have Pin 1 as power (VCC) or common. Signal pins (e.g., LED outputs) often start at Pin 2. When mapping IC outputs to a connector, verify from the connector datasheet which pin is power vs. signal. Example: an 11-pin JST with Pin 1 = COMMON (VCC12) and Pins 2–11 = LED1–LED10 means U6.Pin10 (LED10) goes to J7.Pin2, NOT J7.Pin1. Mapping to Pin 1 would short the LED output to the supply rail.
...
7. Quality checks before responding
   - **Datasheet Citations**: Confirm the `## Datasheet Citations` section lists every IC and non-passive component, with part number and document reference. Every pin assignment in the schematic must be traceable to a cited datasheet.
